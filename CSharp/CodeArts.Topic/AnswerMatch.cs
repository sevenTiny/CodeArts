    ...
    public class AnswerMatch
    {
        ...
        public void Main(string input, string answer)
        {
            ...
            /*
                ...
                ...
            */
            var max = new Tuple<int, List<int>>(0, new List<int>());

            GetMax(0, rec, new bool[inputs.Length], 0, new List<int>(), ref max);

            //输出结果
            Trace.WriteLine($"result={string.Join(",", max.Item2.Select(t => inputs[t]))}");
        }

        private void GetMax(int level, int[][] rec, bool[] hasExist, int lineSum, List<int> selected, ref Tuple<int, List<int>> max)
        {
            //当前层结果
            ...
            for (int i = 0; i < currentLevel.Length; i++)
            {
                ...
                {
                    if (sum > max.Item1)
                        max = new Tuple<int, List<int>>(sum, store);

                    continue;
                }

                GetMax(level + 1, rec, lineExist, sum, store, ref max);
                lineExist[i] = false;
            }
        }
    }
